...
class Channel[Msg] extends InputChannel[Msg] with OutputChannel[Msg] {
   ...
   */
  def !(msg: Msg) {
    recv ! com.twitter.actors.!(this, msg)
  }
  ...
  def send(msg: Msg, replyTo: OutputChannel[Any]) {
    recv.send(com.twitter.actors.!(this, msg), replyTo)
  }
   ...
   */
  def forward(msg: Msg) {
    recv forward com.twitter.actors.!(this, msg)
  }
  ...
  def !?(msg: Msg): Any = {
    ...
    recv.send(com.twitter.actors.!(this, msg), replyCh)
    replyCh.receive {
      case x => x
    }
  }
